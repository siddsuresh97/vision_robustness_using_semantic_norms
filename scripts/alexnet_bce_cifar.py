...
#python vision_robustness_using_semantic_norms/src/alexnet.py --run_name=alexnet_1 --num_classes=86 --weighted_loss=False --alexnet_og_hyperparams=True --device_ids=0,1,2,3 
...
parser.add_argument('--run_name', default="round-1", type=str,
...
CHECKPOINT_DIR = args.output_dir + '/models/{}'.format(args.run_name)  # model checkpoints
...
wandb.init(id = id, resume = "allow", project=args.wandb_project_name, entity="gmishra", settings=wandb.Settings(code_dir="vision_robustness_using_semantic_norms/src/alexnet_mse.py"))
config = wandb.config
