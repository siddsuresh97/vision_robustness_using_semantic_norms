...
if __name__ == '__main__':
    ...
    if args.resume_training:
        ...
        if args.run_id != None:
            ...
            #wandb.run = #wandb.init(resume='allow', id=checkpoint['#wandb_id'])
    ...
    with torch.autograd.set_detect_anomaly(True):
        ...
        with torch.no_grad():
            ...
            for val_imgs, val_classes in val_dataloader:
                ...
                if args.eval == 'euclidean':    
                    val_preds = torch.Tensor([torch.argmin(torch.norm(output[j]-torch.Tensor([leuven_mds_dict[i] for i in val_dataset.classes]).to(device), dim = 1)) for j in range(len(val_output))]).to(device)
                elif args.eval == 'cosine':
                    val_preds = torch.Tensor([torch.argmax(F.cosine_similarity(output[j].unsqueeze(0), torch.Tensor([leuven_mds_dict[i] for i in val_dataset.classes]).to(device), dim = 1)) for j in range(len(val_output))]).to(device)
                else:
                    raise ValueError('Invalid evaluation metric')
            ...
            for test_imgs, test_classes in test_dataloader:
                ...
                if args.eval == 'euclidean':
                    test_preds = torch.Tensor([torch.argmin(torch.norm(output[j]-torch.Tensor([leuven_mds_dict[i] for i in test_dataset.classes]).to(device), dim = 1)) for j in range(len(test_output))]).to(device)
                elif args.eval == 'cosine':
                    test_preds = torch.Tensor([torch.argmax(F.cosine_similarity(output[j].unsqueeze(0), torch.Tensor([leuven_mds_dict[i] for i in test_dataset.classes]).to(device), dim = 1)) for j in range(len(test_output))]).to(device)
                else:
                    raise ValueError('Invalid evaluation metric')
